...
import { Search, Filter, ChevronRight, Clock, User, ArrowRight } from "lucide-react";
      ...
      <div className="transition-all duration-300 px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div className="max-w-7xl mx-auto">
          ...
          {loading ? (
            ...
            >
              {getPostPairs().map((pair, pairIndex) => (
                <div key={pairIndex} className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                  {pair.map((post) => (
                    <motion.article
                      key={post.id}
                      whileHover="hover"
                      variants={cardHoverVariants}
                      className="relative bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl overflow-hidden flex flex-col cursor-pointer border border-slate-700/50 group"
                      style={{
                        backgroundImage: 'radial-gradient(circle at 50% 0%, rgba(120, 100, 250, 0.07), transparent 25%)'
                      }}
                    >
                      {/* Glow effect */}
                      ...
                      <div className="p-5 sm:p-6 flex-grow flex flex-col relative z-10">
                        ...
                        <h2 className="text-xl sm:text-2xl font-bold text-white mb-3 line-clamp-2 group-hover:text-indigo-300 transition-colors duration-300">
                          {post.title}
                        </h2>
                        ...
                        <div className="mt-auto">
                          ...
                          <motion.div
                            ...
                            className="inline-flex items-center text-indigo-400 font-medium text-sm sm:text-base group-hover:text-indigo-300 transition-colors duration-300"
                          >
                            Read more
          ...
          {!loading && filteredPosts.length === 0 && (
            ...
            >
              <h2 className="text-xl sm:text-2xl font-bold text-slate-300 mb-2">No posts found</h2>
